"use strict";
function yourmess(mess) {
    var image = document.getElementById('yourimage').value;
    return `
...
function ConvertTexttoImage(image) {
...
function scrollimage(t) {
    console.log(`${t.scrollTop}  ${t.scrollLeft}`);
}
function simple(image, name, id) {
    return `
...
function list(image, name, id, p) {
    p.classList.add("text-red-500");
    var d = document.querySelector(".cactv");
    var tv = document.querySelector(`.TV${id}`);
    if (!tv) {
        d.innerHTML += simple(image, name, id);
    }
}
function removelist(p, cb) {
    var d = document.querySelector(`.${cb}`);
